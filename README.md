...
==========
  ...
  This document serves as a primer to discuss the important elements of the KPCC iOS codebase as it exists as of the writing of this document in November of 2013. It is meant to be supplemented by reviewing the code itself along with the comments provided within the code.

  The code has been marked up with the token -- Developer Note --  (with hyphens). Doing a project search for that string will return the more important developer comments that are spread out over the code.

  Please also refer to RefactoringProposals.md for notes about where I believe the code could use some refactoring to help simplify the process of debugging and adding to it.

0. KPCC's developer accounts
=============

  KPCC has two discrete Apple Developer Accounts, one is a standard account which we use for all of the distributions into the app store and the other is an Enterprise account which we use to distribute betas and NFR versions. Because we can't reuse the same bundle ID across two accounts this creates a little bit of a dance to do when building the app. Consider the following:

  App Store bundle ID:
  org.scpr.mobile.ipad

  Beta / NFR bundle ID:
  org.scpr.ipad.kpcc

 We subscribe to the paid Parse package which allows us to have more than one APNs certificate on file, so push notifications will appear against both bundle IDs, but just keep in mind that other provisioning care will need to be taken depending on which account you're using.
