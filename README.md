...
The SCPRQueueViewController represents the modal view controller that emerges when the button in the lower-right portion of the screen is tapped. When a piece of audio is added to the queue a CoreData managed object of the class <STRONG>Segment</STRONG> is created and assigned an <STRONG>addedToQueueDate</STRONG> and <STRONG>queuePosition</STRONG>. The Queue is then built based on the presence of these S
