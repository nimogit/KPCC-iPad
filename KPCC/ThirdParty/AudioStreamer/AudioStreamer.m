...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
    ...
    NSString *errString = nil;
      ...
      errString = [AudioStreamer stringForErrorCode:anErrorCode];
			NSLog(@"%@ err: %c%c%c%c %d\n",
    ...
    [self performSelectorOnMainThread:@selector(reportFailure:)
                           withObject:errString
                        waitUntilDone:NO];
...
- (void)reportFailure:(NSString*)error {
  [[AudioManager shared] failAudioWithError:error];
}
...
- (void)adjustVolume {
  OSStatus errorMsg = AudioQueueSetParameter(audioQueue, kAudioQueueParam_Volume,
                                             [[AudioManager shared] currentPlayerVolume]);
  
  if (errorMsg) {
    NSLog(@"AudioQueueSetParameter returned when setting the volume.");
    
    /*[NSTimer scheduledTimerWithTimeInterval:0.1
                                     target:self
                                   selector:@selector(failOverVolume)
                                   userInfo:nil
                                    repeats:NO];*/
  }
}
...
- (void)pause
{
	@synchronized(self)
	{
      ...
      if ( [[AudioManager shared] seeking] ) {
        [[AudioManager shared] setSeeking:NO];
      }
			if (err)
			{
...
- (void)enqueueBuffer
{
    ...
    if ( self.state == AS_PLAYING ) {
      if ( [[AnalyticsManager shared] audioStreamInFailedState] ) {
        [[AnalyticsManager shared] unfailStream];
      }
    }
...
- (void)createQueue
{
  ...
  // Consider modifying this to include a saved player value, with 0.5 as the default if no saved value exists
  [self adjustVolume];
